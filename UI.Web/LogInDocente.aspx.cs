...
namespace UI.Web
{
    public partial class LogInDocente : System.Web.UI.Page
    {
        ...
        protected void ingresarButton_Click(object sender, EventArgs e)
        {
            ...
            if (this.passTextBox.Text == UsuarioActual.Clave && PersonaActual.TipoPersona == Personas.TiposPersonas.docente)
            {
                Session["IDPerDoc"] = PersonaActual.ID;
                Response.Redirect("~/RegistroNotas.aspx");
            }
            else if (this.passTextBox.Text == UsuarioActual.Clave && PersonaActual.TipoPersona != Personas.TiposPersonas.docente)
            {
                Page.Response.Write("El usuario ingresado no pertenece a un docente.");
            }
            else
            {
